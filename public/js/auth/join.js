...
function showMsg(el, valid, msg) {      // 메시지를 띄워주는 담당
	if(valid === true) {
		...
		// $(el).parent().next().removeClass('success').hide().text('');        // warning 메시지를 지워주는 역할(사용x)
	}
	else {
...
function onFocus() {
	...
	for(var i=0; i<$input.length; i++) {
		...
		else if(i > idx) showMsg($input.eq( i ), 'RESET');        // i번째의 밑에 있는 input의  warning에 리셋을 넣어줌
		else showMsg($el);
	}
}
...
function onJoinSubmit(f) {
    ...
    // document.querySelectorAll('form[name="joinForm"] input');       // 배열이 아님
    // Array.from(document.querySelectorAll('form[name="joinForm"] input'));       // 배열로 만들어줌

    // var successChk = true;
    // $('form[name="joinForm"]').find('input').each(function(v){
